...
std::optional<int32_t> FindBestSnap(int32_t edge, std::span<const int32_t> lines,
...
RectPx SnapRectToEdges(RectPx rect, std::span<const int32_t> vertical_edges_px,
                       std::span<const int32_t> horizontal_edges_px,
                       int32_t threshold_px) noexcept {
    if (rect.IsEmpty()) return rect.Normalized();
    ...
    std::optional<int32_t> snap_bottom =
        FindBestSnap(rect.bottom, horizontal_edges_px, threshold_px, false, top);
