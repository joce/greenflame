...
TEST_CASE("BuildDefaultSaveName for region", "[save_image_policy]") {
    ...
    std::wstring const out =
        BuildDefaultSaveName(SaveSelectionSource::Region, std::nullopt, L"", ts);
...
TEST_CASE("BuildDefaultSaveName for monitor", "[save_image_policy]") {
    ...
    std::wstring const out =
        BuildDefaultSaveName(SaveSelectionSource::Monitor, 2, L"", ts);
...
TEST_CASE("BuildDefaultSaveName for window sanitizes title", "[save_image_policy]") {
    ...
    std::wstring const out = BuildDefaultSaveName(SaveSelectionSource::Window,
                                                  std::nullopt, L"My:Window<Name>", ts);
    REQUIRE(out == L"Greenflame-My_Window_Name_-111225-100908");
}

TEST_CASE("BuildDefaultSaveName for empty window title uses default",
          "[save_image_policy]") {
    SaveTimestamp ts = {11, 12, 25, 10, 9, 8};
    std::wstring const out =
        BuildDefaultSaveName(SaveSelectionSource::Window, std::nullopt, L"", ts);
