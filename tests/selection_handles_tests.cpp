...
TEST_CASE("HitTestSelectionHandle hits edge midpoints", "[selection_handles]") {
    ...
    REQUIRE(HitTestSelectionHandle(sel, PointPx{cx, 50}, r) ==
                    SelectionHandle::Top);
    REQUIRE(HitTestSelectionHandle(sel, PointPx{200, cy}, r) ==
                    SelectionHandle::Right);
    REQUIRE(HitTestSelectionHandle(sel, PointPx{cx, 150}, r) ==
                    SelectionHandle::Bottom);
    REQUIRE(HitTestSelectionHandle(sel, PointPx{100, cy}, r) ==
...
TEST_CASE("ResizeRectFromHandle TopLeft moves top-left to cursor",
    ...
    RectPx out =
            ResizeRectFromHandle(anchor, SelectionHandle::TopLeft, cursor);
    REQUIRE(out.left == 80);
    REQUIRE(out.top == 40);
...
TEST_CASE("ResizeRectFromHandle BottomRight moves bottom-right to cursor",
    ...
    RectPx out =
            ResizeRectFromHandle(anchor, SelectionHandle::BottomRight, cursor);
...
TEST_CASE("ResizeRectFromHandle Top edge moves only top", "[selection_handles]") {
    RectPx anchor = RectPx::FromLtrb(100, 50, 200, 150);
    RectPx out =
            ResizeRectFromHandle(anchor, SelectionHandle::Top, PointPx{150, 30});
    REQUIRE(out.left == 100);
    REQUIRE(out.top == 30);
    REQUIRE(out.right == 200);
    REQUIRE(out.bottom == 150);
...
TEST_CASE("ResizeRectFromHandle enforces minimum size", "[selection_handles]") {
    ...
    RectPx out =
            ResizeRectFromHandle(anchor, SelectionHandle::TopRight, cursor);
    REQUIRE(out.Width() >= 1);
