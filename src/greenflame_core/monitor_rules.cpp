...
int32_t DpiToScalePercent(int dpi) noexcept {
    return static_cast<int32_t>((static_cast<int64_t>(dpi) * 100 + 48) / 96);
}

CrossMonitorSelectionDecision DecideCrossMonitorSelection(
...
IndexOfMonitorContaining(PointPx p,
    ...
    for (size_t i = 0; i < monitors.size(); ++i) {
        if (monitors[i].bounds.Contains(p))
            return i;
    }
    return std::nullopt;
}

std::vector<size_t> IndicesOfMonitorsIntersecting(
...
AllowedSelectionRect(RectPx candidate, PointPx start,
    ...
    if (IsAllowed(DecideCrossMonitorSelection(touched_infos)))
        return candidate;

    const std::optional<size_t> start_idx =
            IndexOfMonitorContaining(start, monitors);
    ...
    if (!inter.has_value())
        return RectPx::FromLtrb(0, 0, 0, 0);
    return *inter;
}
} // namespace greenflame::core
