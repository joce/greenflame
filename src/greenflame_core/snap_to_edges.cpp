...
std::optional<int32_t> FindBestSnap(int32_t edge,
                                                                        std::span<const int32_t> lines,
                                                                        int32_t threshold_px,
                                                                        bool require_less_than,
    ...
    for (int32_t line : lines) {
        ...
        if (dist > threshold_px)
            continue;
        if (require_less_than && line >= other_bound)
            continue;
        if (!require_less_than && line <= other_bound)
            continue;
        if (dist < best_dist) {
...
RectPx SnapRectToEdges(RectPx rect,
    ...
    std::optional<int32_t> snap_left =
            FindBestSnap(rect.left, vertical_edges_px, threshold_px, true, right);
    if (snap_left.has_value())
        left = *snap_left;

    std::optional<int32_t> snap_right =
            FindBestSnap(rect.right, vertical_edges_px, threshold_px, false, left);
    if (snap_right.has_value())
        right = *snap_right;

    std::optional<int32_t> snap_top =
            FindBestSnap(rect.top, horizontal_edges_px, threshold_px, true, bottom);
    if (snap_top.has_value())
        top = *snap_top;

    std::optional<int32_t> snap_bottom = FindBestSnap(
            rect.bottom, horizontal_edges_px, threshold_px, false, top);
    if (snap_bottom.has_value())
        bottom = *snap_bottom;

    RectPx out = RectPx::FromLtrb(left, top, right, bottom).Normalized();
